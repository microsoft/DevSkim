...
namespace Microsoft.Security.DevSkim.Tests
{
    ...
    public class UseCaseTest
    {
        ...
        public void UseCase_Normal_Test()
        {
            ...
            Assert.AreEqual(0, matches[0].Index, "strcpy invalid index");
            ...
            Assert.AreEqual(2, matches[0].Index, "todo invalid index");
            ...
            matches = processor.Analyze(testString, "csharp");
            Assert.AreEqual(2, matches.Length, "Same issue should be twice on line");
            Assert.AreEqual(matches[0].Rule, matches[1].Rule, "Same issue should have same rule");

            // Overlap issue
            testString = "            MD5 hash = new MD5CryptoServiceProvider();";
            matches = processor.Analyze(testString, "csharp");
            Assert.AreEqual(3, matches.Length, "Overlap issue count doesn't add up");

            //Override test
            testString = "strncat(dest, \"this is also bad\", strlen(dest))";
            matches = processor.Analyze(testString, new string[] { "c", "cpp" });
            Assert.AreEqual(2, matches.Length, "Override test failed");
        }
        ...
        public void UseCase_IgnoreRules_Test()
        {
            ...
            Assert.AreEqual(15, matches[0].Index, "MD5CryptoServiceProvider invalid index");
            ...
            matches = processor.Analyze(string.Format(testString, expirationDate), "csharp");
            Assert.AreEqual(3, matches.Length, "Expired all should be flagged");
        }
        ...
        public void UseCase_IgnoreSuppression_Test()
        {
            ...
            string testString = "MD5 hash = new MD5CryptoServiceProvider(); //DevSkim: ignore DS126858";
            Match[] matches = processor.Analyze(testString, "csharp");
            Assert.AreEqual(3, matches.Length, "MD5CryptoServiceProvider should be flagged");
            Assert.AreEqual(0, matches[1].Index, "MD5CryptoServiceProvider invalid index");
