...
namespace Microsoft.DevSkim.VSExtension
{
    ...
    internal sealed class DevSkimProvider : IViewTaggerProvider, ITableDataSource
    {
        public string DisplayName
        {
            get
            {
                // This string should, in general, be localized since it is what would be displayed in any UI
                // that lets the end user pick which ITableDataSources should be subscribed to by an instance
                // of the table control. It really isn't needed for the error list however because it
                // autosubscribes to all the ITableDataSources.
                return "DevSkim";
            }
        }
        ...
        {
            // This call can, in theory, happen from any thread so be appropriately thread safe. In practice,
        ...
        {
            // This call will always happen on the UI thread (it is a side-effect of adding or removing the
            // 1st/last tagger).
            lock (_managers)
            {
        ...
        public ITagger<T> CreateTagger<T>(ITextView textView, ITextBuffer buffer) where T : ITag
        {
            ...
            // Only attempt to security check on the view's edit buffer (and multiple views could have that
            // buffer open simultaneously so only create one instance of the security checker.
            if ((buffer == textView.TextBuffer) && (typeof(T) == typeof(IErrorTag)))
            {
                ...
                // This is a thin wrapper around the SkimChecker that can be disposed of without shutting down
                // the SkimChecker (unless it was the last tagger on the skim checker).
        ...
        {
            // This method is called to each consumer interested in errors. In general, there will be only a
            // single consumer (the error list tool window) but it is always possible for 3rd parties to write
            // code that will want to subscribe.
        ...
        private const string _skimCheckerDataSource = "DevSkim";
