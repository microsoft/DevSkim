            ...
            if (sarifLog.Runs.Count > 0)
            {
                ...
                foreach (var resultGroup in groupedResults)
                {
                    ...
                    foreach (var issueRecord in distinctIssueRecords)
                    {
                        ...
                        var ignoreComment = $"{devSkimLanguages.GetCommentPrefix(region.SourceLanguage)} DevSkim: ignore {issueRecord.RulesId} {devSkimLanguages.GetCommentSuffix(region.SourceLanguage)}";
